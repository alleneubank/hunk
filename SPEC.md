...
- **REQ-AGENT-001** — Auto-discovery, when it runs, loads only
  ...
  `.hunk/agent-context.json` is never auto-loaded.
- **REQ-AGENT-002** — `targetId` is the first 12 hex characters of SHA-256 over a
  canonical review-target string: `working-tree` | `staged` | `range`+expression |
  `show`+ref | `stash-show`+ref, each with sorted trimmed pathspecs (stash-show has none).
  Expression and ref are the CLI strings as given, not resolved commit SHAs. Non-repo
  inputs (file compare, patch, stdin) have no conventional path.
- **REQ-AGENT-003** — Resolution order is: `--no-agent-context` (off) → explicit
  `--agent-context <path>` (strict) → config `agent_context` (strict, repo-root relative)
  → keyed conventional path (optional/best-effort) only. No dual bare+keyed auto-read.
- **REQ-AGENT-004** — Optional/conventional load failures (missing or unreadable keyed
  file) yield no agent context and never fail the review. Explicit and config paths remain
  strict.
- **REQ-AGENT-005** — Watch reloads re-resolve the same conventional path for the current
  target and track create/rewrite/delete of that file only for auto-discovery.
- **REQ-AGENT-006** — `hunk review export --json` reports `agentContextPath` as the absolute
  conventional path for this target when auto-discovery applies, else `null`, so agents
  obtain the write path without memorizing the hash. This is the single machine-readable
  agent path surface for the convention.
- **REQ-AGENT-007** — Acceptance for #540: a sidecar written for one range or show target
  does not auto-load on bare working-tree `hunk diff`, and two targets in one repo can each
  keep a sidecar without clobbering. Explicit `--agent-context` still loads any path
  including the legacy bare name.

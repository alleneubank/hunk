...
describe("UI components", () => {
  ...
  test("StatusBar keeps a long notice on one row beside viewed progress", async () => {
    const theme = resolveTheme("github-dark-default", null);
    ...
    // extension toast rendered directly above it off screen.
    const frame = await captureFrame(
      <StatusBar
        filter=""
        filterFocused={false}
    ...
    // The progress segment survives on the same row rather than being pushed off by the notice.
    expect(rows[0]).toContain("viewed 1/3");
  });

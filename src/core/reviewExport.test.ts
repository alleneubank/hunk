...
import { captureReviewCommentAnchor } from "./reviewCommentAnchor";
import { addReviewComment, REVIEW_COMMENTS_VERSION, type ReviewComment } from "./reviewComments";
import { hashPatch } from "./viewedState";
import { HUNK_DIR_NAME, REVIEW_COMMENTS_FILENAME, REVIEW_STATE_FILENAME } from "./paths";
...
describe("review export", () => {
  ...
  test("file identity, order, and hunk ranges match the session projection exactly", () => {
    ...
    const exported = buildReviewExport(bootstrap, { repoRoot });
    const viaSession = buildHunkSessionReview({

 ...
 * what separates them: an added file genuinely has no old side and a deleted file has no
 * new side, so `null` there is the right answer — while `null` for a side that should exist
 * is a failed read, and a client that renders it as empty shows a blank document in place
 * of source it never received.
 */

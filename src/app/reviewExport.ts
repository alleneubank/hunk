...
import { resolveReviewCommentAnchor } from "../core/reviewCommentAnchor";
import {
...
import { conventionalAgentContextPath } from "../core/run/paths";
import { resolveReviewCommentsPath, resolveViewedStatePath } from "../core/reviewStore";
...
import { readViewedState, resolveViewedPaths } from "../core/viewedState";
...
export interface ReviewExport {
  ...
  /**
   * Absolute conventional auto-discovery path for this review's target, when one exists.
   *
   * Agents write notes here so bare `hunk diff` / range / show load them without flags.
   * Null when the input is not repo-backed or auto-discovery does not apply. Never the
   * bare `agent-context.json` name.
   */
...
export function buildReviewExport(
  ...
  if (includePatch) {
    const patchById = new Map(bootstrap.changeset.files.map((file) => [file.id, file.patch]));
    review.files = review.files.map((file) => {
      const patch = patchById.get(file.id);
  ...
  const agentContextPath =
    repoRoot && bootstrap.input.options.noAgentContext !== true
      ? conventionalAgentContextPath(repoRoot, bootstrap.input)
      : null;

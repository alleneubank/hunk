 ...
 *
 * Never derive `<base>` from `git describe --tags --abbrev=0`: once `-fork` tags
 * accumulate, describe matches those and the version doubles
 * (`0.18.0-fork.…gabc-fork.…gdef`). The field here must match the nearest plain
 * upstream `vX.Y.Z` tag.
 */

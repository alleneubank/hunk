 ...
 *
 * Tagging is gated on the base ref's CI being observed green, because the tag is what
 * ships a binary to every fleet host — see ciVerdictForSha.
 *
 * Usage:
 ...
 *   bun run scripts/release-fork.ts --run --allow-red-ci   # cut anyway (deliberate dogfood)
 */
...
const FORK_OWNED_FILES = [
  ...
  // Fork dogfood release SOP lives under ## fork dogfood releases — keep it across rebuilds.
  "AGENTS.md",
];
...
function parseArgs(argv: string[]): Options {
  const options: Options = {
    ...
    base: "main",
    forkBranch: "fork",
    // `origin` is the personal fork remote; `upstream` is modem-dev. Override with --remote.
    remote: "origin",
    allowRedCi: false,
    ciWorkflow: "Main CI",
  };
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (arg === "--allow-red-ci") options.allowRedCi = true;
    else if (arg === "--ci-workflow") options.ciWorkflow = takeValue();
...
/** GitHub `owner/repo` for a remote, so CI is queried against the fork and never upstream. */
...
type CiState = "green" | "red" | "pending" | "missing" | "unavailable";
...
/**
 * Verdict of `workflow` for one commit on the fork remote.
 *
 * Fail-closed by construction: only an observed `completed`/`success` returns green. A run
 * that is missing, still going, or unreadable because `gh` is absent or unauthenticated is
 * NOT green — silently passing here would manufacture exactly the confidence the gate
 * exists to deny, since the tag ships a binary to every fleet host.
 */
function ciVerdictForSha(repoSlug: string, workflow: string, sha: string): CiVerdict {
  ...
  if (proc.exitCode !== 0) {
    const stderr = proc.stderr.toString().trim();
    return { state: "unavailable", detail: stderr || `gh exited ${proc.exitCode}` };
  }

  let runs: { databaseId: number; status: string; conclusion: string | null }[];
  try {
    runs = JSON.parse(proc.stdout.toString());
  } catch (error) {
    return { state: "unavailable", detail: `unreadable gh output: ${String(error)}` };
  }

  // gh returns newest first, so a re-run supersedes the original verdict.
  const latest = runs[0];
  if (!latest) {
    return { state: "missing", detail: `no '${workflow}' run for ${sha.slice(0, 7)}` };
  }
  if (latest.status !== "completed") {
    return { state: "pending", detail: `run ${latest.databaseId} is ${latest.status}` };
  }
  if (latest.conclusion !== "success") {
    return { state: "red", detail: `run ${latest.databaseId} concluded ${latest.conclusion}` };
  }
  return { state: "green", detail: `run ${latest.databaseId} succeeded` };
}
...
async function main() {
  ...
  // Use the fully qualified local ref for the fork branch so Git DWIM cannot
  // confuse it with a same-named remote-tracking branch.
  ...
  // Full sha: `gh run list --commit` matches headSha exactly, not an abbreviation.
  const repoSlug = repoSlugForRemote(options.remote);
  const ci = ciVerdictForSha(repoSlug, options.ciWorkflow, git(["rev-parse", options.base]));

  console.log(`base ref     : ${options.base} (${baseSha})`);
  console.log(`base version : ${baseVersion}`);
  console.log(`fork version : ${newVersion}`);
  console.log(`fork tag     : ${newTag}`);
  console.log(`base CI      : ${ci.state} — ${ci.detail} [${options.ciWorkflow} @ ${repoSlug}]`);
  ...
  if (ci.state !== "green" && !options.allowRedCi) {
    throw new Error(
      `Refusing to tag ${newTag}: ${options.ciWorkflow} on ${options.base} (${baseSha}) is ` +
        `${ci.state} — ${ci.detail}. The tag is what ships a binary to every fleet host, so it ` +
        `needs a base that was observed green. Land a fix on ${options.base} and re-run, wait ` +
        `for a pending run to finish, or pass --allow-red-ci for a deliberate dogfood cut.`,
    );
  }
  if (ci.state !== "green") {
    console.log(`\n! --allow-red-ci: cutting ${newTag} from a ${ci.state} base (${ci.detail}).`);
  }

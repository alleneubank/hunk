...
- **Tarballs:** host platform only by default — `hunkdiff-<os>-<arch>.tar.gz` with `hunk`,
  `skills/`, `metadata.json` at the archive root, plus `checksums.txt` (sha256sum). Match prior
  fleet layout so mise keeps working.
